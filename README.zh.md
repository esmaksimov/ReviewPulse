...
**2. 添加机器人**：把它设为评审频道的管理员——授予**发布消息**权限（供 `/announce` 使用）和**删除消息**权限（让已关闭评审的帖子自动从频道消失）——**同时**加入其关联的讨论组（评论功能必须开启）。
...
- **关闭评审（达到所需通过数，或点击“🗄 关闭”按钮）会删除频道里的帖子**——不是立刻删除，而是等 `CHANNEL_CLEANUP_DELAY_HOURS`（默认 4 小时）之后，让帖子在关闭后还能保留一段时间。讨论串（包括卡片和所有评论）不受影响，只是频道列表会变干净。需要机器人拥有管理员的**删除消息**权限；没有的话，关闭操作仍会发生，只是帖子会继续留在频道里。
